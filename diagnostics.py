...
class AGiXTDiagnostics:
    ...
    async def check_database_connections(self):
        ...
        try:
            ...
            pending_tasks = (
                self.session.query(TaskItem)
                .filter(TaskItem.completed == False, TaskItem.scheduled == True)
                .count()
            )
    ...
    def check_system_resources(self):
        ...
        try:
            ...
            for proc in psutil.process_iter(
                ...
                try:
                    if proc.info["cmdline"] and any(
                        "agixt" in str(cmd).lower() for cmd in proc.info["cmdline"]
                    ):
            ...
            for proc in agixt_processes:
                memory_mb = (
                    proc["memory_info"].rss / 1024 / 1024 if proc["memory_info"] else 0
                )
    ...
    async def check_async_tasks(self):
        ...
        try:
            ...
            for name, count in sorted(
                task_names.items(), key=lambda x: x[1], reverse=True
            ):
                print(f"  {name}: {count}")
                    ...
                    try:
                        with open(log_file, "r") as f:
                            ...
                            for pattern in error_patterns:
                                ...
                                if count > 0:
                                    print(
                                        f"  {log_file.name}: {pattern} found {count} times"
                                    )
                    except Exception as e:

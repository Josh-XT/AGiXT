            ...
            try:
                if (provider == gpt4free.Provider.UseLess):
                    response = gpt4free.Completion.create(
                        provider, prompt=prompt, model=self.AI_MODEL, temperature=self.AI_TEMPERATURE, systemMessage=""
                    )
                    if ("text" in response):
                    ...
                    response = gpt4free.Completion.create(
                        provider, prompt=prompt, token=token, model=self.AI_MODEL, temperature=self.AI_TEMPERATURE
                    )
                    ...
                    response = gpt4free.Completion.create(
                        provider, prompt=prompt, model=self.AI_MODEL, temperature=self.AI_TEMPERATURE
                    )
                if (not response):

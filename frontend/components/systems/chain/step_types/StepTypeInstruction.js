...
export default function StepTypeTask({agent_name, prompt}) {
    ...
    return <>
        <Select label="Agent" sx={{ mx: "0.5rem" }} value={agent} onChange={(e) => setAgent(e.target.value)}>
            <MenuItem value={-1}>Select an Agent...</MenuItem>
            {agents?.data?.map((agent, index) => {
                return <MenuItem key={index} value={index}>{agent.name}</MenuItem>;
            })}
        </Select>
        <TextField label="Instruction" multiline lines={20} sx={{ mx: "0.5rem", flex: 1 }} value={instruction} onChange={(e) => setInstruction(e.target.value)} />
    </>;
}
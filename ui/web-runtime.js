 ...
 * small Tauri IPC/event surface it expects, backed by explicit localStorage
 * state, web auth, and AGiXT's HTTP/SSE/WebSocket APIs.
 */
(function () {
  'use strict';
  ...
  function shouldUseConfiguredServerUrl(value) {
    if (!value) return false;
    try {
      const configured = new URL(value, origin);
      const page = new URL(origin);
      if (isLoopbackHost(configured.hostname) && !isLoopbackHost(page.hostname)) {
        return false;
      }
  ...
  function currentServerUrl() {
    return trimSlash(shouldUseConfiguredServerUrl(config.serverUrl) ? config.serverUrl : origin);
  }

  function serverUrlOrCurrent(value) {
    return trimSlash(shouldUseConfiguredServerUrl(value) ? value : currentServerUrl());
  }

  function currentWebUrl() {
    return trimSlash(origin || config.webUrl);
  }
  ...
  function extractJwt(raw) {
    ...
    try {
      ...
      for (const key of ['token', 'jwt', 'authorization', 'auth']) {
    ...
    const match = trimmed.match(/(?:^|[?&#\s])(token|jwt|authorization|auth)=([^&\s]+)/i);
    if (!match || !match[2]) return null;
    try {
      const value = decodeURIComponent(match[2]);
      return value.startsWith('eyJ') && value.split('.').length === 3 ? value : null;
    } catch (_) {
      return match[2].startsWith('eyJ') && match[2].split('.').length === 3 ? match[2] : null;
    }
  }
  ...
  function rememberOAuthFlow(args, connectFlow) {
    ...
    const providerName = provider.name || args.provider_name || args.provider || '';
    try {
      window.localStorage.setItem(OAUTH_FLOW_KEY, JSON.stringify({
        ...
        provider_endpoint_slug: oauthEndpointSlugFor(providerName),
        provider_redirect_slug: redirectSlugFor(providerName),
        connect: !!connectFlow,
        started_at: Date.now(),
      }));
    } catch (_) {}
  ...
  function pendingOAuthFlow() {
    ...
    return {
      ...
      provider_endpoint_slug: flow.provider_endpoint_slug || '',
      provider_redirect_slug: flow.provider_redirect_slug || '',
  ...
  async function loginWithJwt(args) {
    const token = extractJwt(args.raw || args.jwt || args.token || args.authorization);
    if (!token) throw toolError("couldn't find a JWT in that input");
    const settings = loadSettings();
    const serverUrl = trimSlash(args.serverUrl || args.server_url || settings.server_url || currentServerUrl());
    const user = await jsonFetch(`${serverUrl}/v1/user`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}` },
    });
    const email = user && (user.email || user.user_email || user.username || decodeJwtEmail(token));
    rememberRuntimeAuth({ jwt: token, user_email: email });
    saveSettings({
      ...settings,
      server_url: serverUrl,
  ...
  function redirectSlugFor(providerName) {
    let s = String(providerName || '').trim().toLowerCase()
      .replace(/[-.]/g, '_')
      .replace(/\s+/g, '_');
    ...
    for (const [slug, aliases] of Object.entries(families)) {
      if (aliases.includes(s)) return slug;
    }
    return s.replace(/_/g, '-');
  }

  function oauthEndpointSlugFor(providerName) {
    let s = String(providerName || '').trim().toLowerCase();
    if (s.endsWith('_sso')) s = s.slice(0, -4);
  ...
  async function buildOAuthUrl(args, connectFlow) {
    ...
    // In the browser-hosted build the OAuth callback must return to the
    // actual origin serving this SPA. The service picker may still show
    // canonical production brands, but using those brand URLs here causes
    // redirect_uri_mismatch on staged/custom domains.
    const callbackWebUrl = args.redirect_uri
      ? trimSlash(args.web_url || currentWebUrl())
      : currentWebUrl();
    const redirectUri = args.redirect_uri || `${callbackWebUrl}/user/close/${slug}`;
    let pkce = null;
    ...
    rememberOAuthFlow({ ...args, web_url: callbackWebUrl }, connectFlow);
  ...
  async function invoke(command, payload) {
    ...
    switch (command) {
      ...
      case 'login_with_jwt':
        return loginWithJwt(args.args || args);
  ...
  function openUrl(url) {
    ...
    const opened = window.open(text, '_blank');
    if (opened) {
  ...
  async function handleOAuthClose() {
    ...
    const providerForPost = (oauthFlow && oauthFlow.provider_endpoint_slug) || providerFromPath;
    const referrerSlug = (oauthFlow && oauthFlow.provider_redirect_slug) || providerFromPath;
    const headers = { 'Content-Type': 'application/json' };
    if (isConnect && callbackSettings.jwt) Object.assign(headers, authHeader(callbackSettings));
    const body = {
      code,
      referrer: `${trimSlash(callbackSettings.web_url || currentWebUrl())}/user/close/${referrerSlug}`,
    };

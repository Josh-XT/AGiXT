...
{"action":"failed","summary":"reason"}`;
  ...
  async function xtschoolBridge() {
    if (window.XTSchool) return window.XTSchool;
    if (window.AgixtSidenav && typeof window.AgixtSidenav.setActiveView === 'function') {
      try { window.AgixtSidenav.setActiveView('xtschool'); } catch (_) {}
    }
    for (let i = 0; i < 24; i += 1) {
      await delay(50);
      if (window.XTSchool) return window.XTSchool;
    }
    throw new Error('XTSchool is not available in this session.');
  }

  async function runXTSchoolTool(method, args) {
    const bridge = await xtschoolBridge();
    if (!bridge || typeof bridge[method] !== 'function') {
      throw new Error(`XTSchool bridge method missing: ${method}`);
    }
  ...
  async function execute(action) {
    ...
    try {
      ...
      switch (name) {
        ...
        case 'xtschool_search_library':
        case 'search_xtschool':
        case 'search_kid_library':
          return await runXTSchoolTool('searchLibrary', a);

        case 'xtschool_open_content':
        case 'open_xtschool_content':
        case 'play_video':
        case 'play_music':
        case 'open_book':
        case 'open_game':
          return await runXTSchoolTool('openContent', {
            ...a,
            content_id: a.content_id || a.video_id || a.track_id || a.asin || a.game_id || a.id,
          });

        case 'xtschool_import_content':
        case 'xtschool_add_content_url':
        case 'add_youtube_url':
        case 'add_spotify_url':
        case 'approve_audiobook':
          return await runXTSchoolTool('importContent', {
            ...a,
            url: a.url || a.source_url || a.href || a.link || (a.asin ? `https://www.audible.com/pd/${a.asin}` : ''),
            approved: a.approved != null ? a.approved : true,
          });

        case 'xtschool_update_content':
        case 'xtschool_approve_content':
        case 'approve_xtschool_content':
          return await runXTSchoolTool(
            name === 'xtschool_update_content' ? 'updateContent' : 'approveContent',
            {
              ...a,
              content_id: a.content_id || a.video_id || a.track_id || a.asin || a.id,
            },
          );

        case 'xtschool_set_content_bedtime':
        case 'set_video_bedtime':
        case 'set_music_bedtime':
          return await runXTSchoolTool('setContentBedtime', {
            ...a,
            content_id: a.content_id || a.video_id || a.track_id || a.id,
          });

        case 'xtschool_create_playlist':
        case 'create_admin_playlist':
          return await runXTSchoolTool('createPlaylist', a);

        case 'xtschool_add_content_to_playlist':
        case 'add_video_to_admin_playlist':
          return await runXTSchoolTool('addContentToPlaylist', a);

        case 'xtschool_open_creation':
        case 'open_html_creation':
          return await runXTSchoolTool('openCreation', a);

        case 'xtschool_create_html_creation':
        case 'create_html_creation':
          return await runXTSchoolTool('createHtmlCreation', a);

        case 'xtschool_update_html_creation':
        case 'update_html_creation':
          return await runXTSchoolTool('updateHtmlCreation', a);

        case 'xtschool_create_calendar_event':
        case 'create_calendar_event':
          return await runXTSchoolTool('createCalendarEvent', a);

        case 'xtschool_media_control':
        case 'media_control':
        case 'game_control':
          return await runXTSchoolTool('mediaControl', a);

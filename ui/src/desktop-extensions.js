...
(function () {
  ...
  const SIDENAV_ICONS = {
    ...
    school:      '<path d="m22 10-10-5-10 5 10 5 10-5z"/><path d="M6 12v5c0 1.5 3 3 6 3s6-1.5 6-3v-5"/><path d="M22 10v6"/>',
  };
  ...
  function unmountAndForget(id) {
    ...
    const btn = document.querySelector(
      `.sidenav .sidenav-btn[data-view="${cssEscape(id)}"]`,
    );
  ...
  async function refresh(opts) {
    ...
    const data = await fetchManifest({ force });

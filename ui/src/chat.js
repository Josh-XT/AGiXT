...
(function () {
  ...
  function adoptTransientActivity(serverId, parsed, msg) {
    ...
    if (titleEl && parsed.label && !finalized) titleEl.textContent = parsed.label;
    if (finalized) refreshActivityTitle(entry.el);
  ...
  // every subactivity / CLIENT_TOOL marker inside the body. Surfaces as
  // "Activities [N]" next to the elapsed clock once the block finalizes.
  function countActivityActions(elm) {
    if (!elm) return 0;
    const merged = Number(elm.dataset.mergedCount);
    const base = Number.isFinite(merged) && merged > 0 ? merged : 1;
    const subactivities = elm.querySelectorAll('.subactivity').length;
    const toolGroups = elm.querySelectorAll('.sub-tool-group').length;
    return base + subactivities + toolGroups;
  }

  function refreshActivityTitle(elm) {
    if (!elm) return;
    if (elm.getAttribute('data-finalized') !== 'true') return;
    const titleEl = elm.querySelector('.activity-title');
    if (!titleEl) return;
    const count = countActivityActions(elm);
    titleEl.textContent = count > 0 ? `Activities [${count}]` : 'Activities';
  }
  ...
  function ingest(msg, isInitial) {
    ...
    if (parsed.kind === 'activity') {
      ...
      if (shouldMergeThinking) {
        ...
        if (merged.el) {
          const prev = Number(merged.el.dataset.mergedCount);
          merged.el.dataset.mergedCount = String((Number.isFinite(prev) && prev > 0 ? prev : 1) + 1);
          refreshActivityTitle(merged.el);
        }

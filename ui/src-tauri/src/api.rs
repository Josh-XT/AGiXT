...
/// AGiXT's OAuth apps are registered against stable provider-family
/// callback slugs. Product-specific providers such as `microsoft_email`
/// and `microsoft_calendar` still use `/user/close/microsoft` because the
/// Azure application only has that redirect URI configured. Unknown custom
/// providers keep the old normalization behavior.
pub fn redirect_slug_for(provider_name: &str) -> String {
    ...
    let canonical = lower
        ...
        .collect::<Vec<_>>()
        .join("_");
    match canonical.trim_end_matches("_sso") {
        "microsoft"
        | "microsoft_365"
        | "microsoft_calendar"
        | "microsoft_email"
        ...
        | "office_365"
        | "outlook"
        | "teams" => "microsoft".to_string(),
        "google" | "google_calendar" | "google_email" | "google_marketing" | "youtube" => {
            "google".to_string()
        }
        "github" | "github_copilot" => "github".to_string(),
...
/// Slug used for AGiXT API endpoint paths such as `/v1/oauth2/{provider}`.
/// Unlike callback redirects, these preserve product-specific providers so
/// the backend can apply the right scopes and credential records.
pub fn oauth_endpoint_slug_for(provider_name: &str) -> String {
    let lower = provider_name.trim().to_ascii_lowercase();
...
mod tests {
    ...
    fn redirect_slug_strips_sso_suffix_and_normalizes_separators() {
        ...
        assert_eq!(redirect_slug_for("microsoft_email"), "microsoft");
        assert_eq!(redirect_slug_for("microsoft-calendar"), "microsoft");
        assert_eq!(redirect_slug_for("microsoft_onedrive"), "microsoft");
        assert_eq!(redirect_slug_for("teams"), "microsoft");
        assert_eq!(redirect_slug_for("github_sso"), "github");
        assert_eq!(redirect_slug_for("github_copilot"), "github");
        assert_eq!(redirect_slug_for("google_sso"), "google");
        assert_eq!(redirect_slug_for("google_email"), "google");
        assert_eq!(redirect_slug_for("google calendar"), "google");
        assert_eq!(redirect_slug_for("youtube"), "google");
        // Unknown providers keep their name, but separators are normalized
        // to dashes.
    ...
    fn redirect_uri_should_be_web_not_backend() {
        ...
        // Caller will pass the already-normalized web callback as the
        // redirect URI — confirm we forward exactly that without
        // sneaking in the backend URL anywhere.
        let redirect = "https://josh.devxt.com/user/close/microsoft";
        let url = build_oauth_url(&p, redirect, None);
        let encoded = "redirect_uri=https%3A%2F%2Fjosh.devxt.com%2Fuser%2Fclose%2Fmicrosoft";
        assert!(

...
async fn build_oauth_login_url(args: OAuthUrlArgs) -> ToolResult<OAuthUrlResult> {
    ...
    // pre-registered. The slug is *not* always the raw provider name:
    // Microsoft variants use `/microsoft`, Google variants use `/google`,
    // and custom providers fall back to normalized dashes. See
    // `api::redirect_slug_for` for the exact rules.
    //
...
async fn handle_deep_link_oauth_connect(
    ...
    let redirect_slug = api::redirect_slug_for(&provider);
    let endpoint_slug = api::oauth_endpoint_slug_for(&provider);
    let referrer = format!(
        "{}/user/close/{}",

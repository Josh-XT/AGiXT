...
const ROLE_NAMES = { 0: 'Super Admin', 1: 'Owner', 2: 'Admin', 3: 'User', 4: 'Child', 5: 'Chat User', 6: 'Read Only' };
// Roles assignable from the invite/role-change UI. Super admin (0) and
// tenant admin/owner (1) remain protected, while Child is intentionally
// assignable so parents can enable the child-mode shell and controls.
const ASSIGNABLE_ROLE_IDS = [2, 3, 4, 5, 6];
...
const CHILD_FEATURE_KEYS = ['videos', 'music', 'audiobooks', 'games', 'chat'];
const CHILD_FEATURE_LABELS = {
  app: 'Whole app',
  videos: 'Videos',
  music: 'Music',
...
CompaniesView.prototype.refresh = async function () {
  try {
    ...
    await this.enrichCompanyBillingBalances(this.companies);
...
CompaniesView.prototype.enrichCompanyBillingBalances = async function (companies) {
  ...
  await Promise.all(rows.map(async (c) => {
    ...
    try {
      const [limitsResult, balanceResult] = await Promise.allSettled([
        this.fetchJson('/v1/billing/plan/limits?company_id=' + encodeURIComponent(c.id)),
        this.fetchJson('/v1/billing/tokens/balance?company_id=' + encodeURIComponent(c.id) + '&sync=true'),
      ]);
      const data = limitsResult.status === 'fulfilled' ? limitsResult.value : null;
      const limits = data && data.limits ? data.limits : {};
      const usage = data && data.usage ? data.usage : {};
      const periodTokens = Number(limits.tokens_per_month || 0);
      const topupTokens = Number(usage.token_balance || 0);
      const usedThisPeriod = Number(usage.tokens_this_period || usage.tokens_used_this_period || 0);
      if (periodTokens > 0 || topupTokens > 0 || usedThisPeriod > 0) {
        c.token_limit = periodTokens;
        c.topup_token_balance = topupTokens;
        c.tokens_this_period = usedThisPeriod;
        c.token_balance = Math.max(0, periodTokens + topupTokens - usedThisPeriod);
        c.plan_name = data.plan_name || c.plan_name;
      }
      if (balanceResult.status === 'fulfilled' && balanceResult.value) {
        const balance = balanceResult.value;
        if (balance.token_balance != null) {
          c.token_balance = Number(balance.token_balance || 0);
          c.token_balance_usd = Number(balance.token_balance_usd || 0);
        }
        if (balance.tokens_used_total != null) {
          c.tokens_used_total = Number(balance.tokens_used_total || 0);
        }
      }
    } catch (_) {
      // Billing may be disabled or unavailable to this member. The company
      // list should still render with whatever values /v1/user supplied.
    }
  }));
};
...
CompaniesView.prototype.loadDetailData = async function () {
  ...
  const [members, invitations, defaultRoles, customRoles, childControls] = await Promise.all([
    ...
    this.fetchJson('/v1/roles?company_id=' + encodeURIComponent(cid))
      .then((d) => (d && d.roles) || [])
      .catch(() => []),
    this.fetchJson('/v1/companies/' + encodeURIComponent(cid) + '/child-controls')
      .catch(() => null),
  ]);
  let resolvedMembers = Array.isArray(members) ? members : [];
  if (!resolvedMembers.length && this.detail.company && Array.isArray(this.detail.company.users)) {
    resolvedMembers = this.detail.company.users;
  }
  this.detail.members = resolvedMembers;
  ...
  this.detail.childControls = childControls || null;
...
CompaniesView.prototype._buildTools = function () {
  ...
  if (Number(this.roleId) === 0 || Number(this.roleId) === 1) {
...
CompaniesView.prototype.rowHtml = function (c) {
  ...
  const tokens = formatCompanyTokenBalanceC(c);
...
CompaniesView.prototype.renderDetailBody = function () {
  ...
  if (this.canAdminCompany(c)
      && this.detail.childControls
      && Array.isArray(this.detail.childControls.children)
      && this.detail.childControls.children.length) {
    this.detailBodyEl.appendChild(this.buildChildControlsCard(c));
  }
...
CompaniesView.prototype.buildCompanyInfoCard = function (c) {
  ...
  card._body.appendChild(makeKvC([
    ...
    ['Token balance', formatCompanyTokenBalanceC(c)],
    ['Address',     c.address],
    ['City',        c.city],
    ['State',       c.state],
    ['Zip',         c.zip_code],
...
CompaniesView.prototype.buildChildControlsCard = function (c) {
  const children = (this.detail.childControls && this.detail.childControls.children) || [];
  const card = makeCardC('Parental controls', children.length + ' child profile(s)');
  const body = card._body;
  const view = this;
  ...
  function defaultRule() {
    return { enabled: true, schedule: null, free_weekdays: [], override_until: null };
  }
  function normalizeRule(raw) {
    const out = defaultRule();
    raw = raw && typeof raw === 'object' ? raw : {};
    if (Object.prototype.hasOwnProperty.call(raw, 'enabled')) out.enabled = !!raw.enabled;
    if (raw.schedule && typeof raw.schedule === 'object') {
      out.schedule = {
        lock_from: raw.schedule.lock_from || '21:00',
        lock_to: raw.schedule.lock_to || '07:00',
        mode: raw.schedule.mode === 'bedtime' ? 'bedtime' : 'locked',
      };
    }
    if (Array.isArray(raw.free_weekdays)) {
      out.free_weekdays = Array.from(new Set(raw.free_weekdays
        .map((d) => Number(d))
        .filter((d) => Number.isInteger(d) && d >= 0 && d <= 6))).sort((a, b) => a - b);
    }
    if (raw.override_until) out.override_until = String(raw.override_until);
    return out;
  }
  function normalizeRestrictions(raw) {
    raw = raw && typeof raw === 'object' ? raw : {};
    const features = {};
    CHILD_FEATURE_KEYS.forEach((key) => {
      features[key] = normalizeRule(raw.features && raw.features[key]);
    });
    return { app: normalizeRule(raw.app), features };
  }
  function activeText(access) {
    if (!access) return 'Allowed';
    if (access.locked) return access.reason === 'scheduled' ? 'Scheduled lock active' : 'Locked';
    if (access.bedtime) return 'Bedtime mode active';
    if (access.reason === 'override_active') return 'Temporary unlock active';
    if (access.reason === 'free_day') return 'Free day';
    return 'Allowed';
  }
  function makeRuleEditor(label, rule, access, allowBedtime) {
    rule = normalizeRule(rule);
    const wrap = document.createElement('div');
    wrap.className = 'co-child-rule';
    const head = document.createElement('div');
    head.className = 'co-child-rule-head';
    head.innerHTML =
      '<div><strong>' + escapeC(label) + '</strong>' +
      (rule.override_until ? '<span class="co-badge success">Override</span>' : '') +
      '</div><span class="co-child-state">' + escapeC(activeText(access)) + '</span>';
    ...
    enabledLabel.className = 'co-check co-child-inline';
    enabledLabel.appendChild(enabled);
    enabledLabel.appendChild(document.createTextNode('Available when no schedule is locking it'));
    wrap.appendChild(enabledLabel);

    const hasSchedule = document.createElement('input');
    hasSchedule.type = 'checkbox';
    hasSchedule.checked = !!rule.schedule;
    hasSchedule.dataset.ruleField = 'schedule_enabled';
    const scheduleToggle = document.createElement('label');
    scheduleToggle.className = 'co-check co-child-inline';
    scheduleToggle.appendChild(hasSchedule);
    scheduleToggle.appendChild(document.createTextNode('Use scheduled quiet hours'));
    wrap.appendChild(scheduleToggle);

    const schedule = rule.schedule || { lock_from: '21:00', lock_to: '07:00', mode: 'locked' };
    const grid = document.createElement('div');
    grid.className = 'co-child-rule-grid';
    const from = document.createElement('input');
    from.type = 'time'; from.className = 'co-input'; from.value = schedule.lock_from || '21:00';
    from.dataset.ruleField = 'lock_from';
    const to = document.createElement('input');
    to.type = 'time'; to.className = 'co-input'; to.value = schedule.lock_to || '07:00';
    to.dataset.ruleField = 'lock_to';
    const mode = document.createElement('select');
    mode.className = 'co-select';
    mode.dataset.ruleField = 'mode';
    mode.innerHTML = '<option value="locked">Locked</option><option value="bedtime">Bedtime mode</option>';
    mode.value = allowBedtime && schedule.mode === 'bedtime' ? 'bedtime' : 'locked';
    mode.disabled = !allowBedtime;
    grid.appendChild(wrapField('From', from));
    grid.appendChild(wrapField('To', to));
    grid.appendChild(wrapField('Mode', mode, allowBedtime ? '' : 'Whole-app schedules always lock.'));
    wrap.appendChild(grid);

    const days = document.createElement('div');
    days.className = 'co-child-days';
    const dayLabels = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    const dayChecks = [];
    dayLabels.forEach((day, index) => {
      const cb = document.createElement('input');
      cb.type = 'checkbox';
      cb.checked = rule.free_weekdays.includes(index);
      dayChecks.push(cb);
      const labelEl = document.createElement('label');
      labelEl.appendChild(cb);
      labelEl.appendChild(document.createTextNode(day));
      days.appendChild(labelEl);
    });
    const daysWrap = wrapField('Free days', days, 'Schedules are ignored on checked days.');
    wrap.appendChild(daysWrap);

    function syncScheduleInputs() {
      const on = hasSchedule.checked;
      from.disabled = !on;
      to.disabled = !on;
      mode.disabled = !on || !allowBedtime;
    }
    hasSchedule.addEventListener('change', syncScheduleInputs);
    syncScheduleInputs();
    ...
    return {
      ...
      read() {
        return {
          ...
          schedule: hasSchedule.checked
            ? { lock_from: from.value || '21:00', lock_to: to.value || '07:00', mode: mode.value || 'locked' }
            : null,
          free_weekdays: dayChecks.map((cb, index) => cb.checked ? index : null).filter((v) => v != null),
          override_until: rule.override_until || null,
        };
      },
    };
  }
  async function saveChild(child, contextInput, editors) {
    const restrictions = { app: editors.app.read(), features: {} };
    CHILD_FEATURE_KEYS.forEach((key) => { restrictions.features[key] = editors[key].read(); });
    await view.fetchJson(
      '/v1/companies/' + encodeURIComponent(c.id) + '/child-controls/' + encodeURIComponent(child.id),
      {
        method: 'PATCH',
        json: {
          ai_context: contextInput.value || '',
          restrictions,
        },
      },
    );
  ...
  async function setOverride(child, feature, minutes) {
    const path = '/v1/companies/' + encodeURIComponent(c.id)
      + '/child-controls/' + encodeURIComponent(child.id) + '/override';
    if (minutes) {
      await view.fetchJson(path, {
        method: 'POST',
        json: { feature, until: new Date(Date.now() + minutes * 60000).toISOString() },
      });
    } else {
      await view.fetchJson(path + '?feature=' + encodeURIComponent(feature), { method: 'DELETE' });
    }
    await view.loadDetailData();
    view.renderDetailBody();
  }
  ...
  children.forEach((child) => {
    const restrictions = normalizeRestrictions(clone(child.restrictions));
    const access = (child && child.access) || {};
    const section = document.createElement('div');
    section.className = 'co-child-section';
    const name = ((child.first_name || '') + ' ' + (child.last_name || '')).trim()
      || child.email || 'Child';
    section.innerHTML =
      '<div class="co-child-title-row">' +
        '<div><div class="co-list-item-title">' + escapeC(name) + '</div>' +
        '<div class="co-list-item-meta">' + escapeC(child.email || '') + '</div></div>' +
      '</div>';

    const contextInput = document.createElement('textarea');
    contextInput.className = 'co-input co-textarea';
    contextInput.rows = 4;
    contextInput.value = child.ai_context || child.mandatory_context || '';
    contextInput.dataset.childControlContext = child.id;
    section.appendChild(wrapField('Mandatory agent context', contextInput,
      'This is attached to every child chat turn for this company.'));

    const editors = {};
    editors.app = makeRuleEditor('Whole app', restrictions.app, access.app, false);
    editors.app.el.dataset.childRule = child.id + ':app';
    section.appendChild(editors.app.el);
    CHILD_FEATURE_KEYS.forEach((key) => {
      const allowBedtime = key === 'videos' || key === 'music' || key === 'audiobooks';
      editors[key] = makeRuleEditor(
        CHILD_FEATURE_LABELS[key] || key,
        restrictions.features[key],
        access.features && access.features[key],
        allowBedtime,
      );
      editors[key].el.dataset.childRule = child.id + ':' + key;
      section.appendChild(editors[key].el);
    });

    const actions = document.createElement('div');
    actions.className = 'co-child-actions';
    const save = document.createElement('button');
    save.type = 'button'; save.className = 'co-primary'; save.textContent = 'Save controls';
    save.addEventListener('click', async () => {
      try {
        await saveChild(child, contextInput, editors);
        toastInfo('Parental controls saved');
      } catch (err) { view.renderError(err); }
    });
    actions.appendChild(save);
    [
      ['Unlock app 30m', 'app', 30],
      ['Unlock chat 30m', 'chat', 30],
      ['Clear app override', 'app', 0],
      ['Clear chat override', 'chat', 0],
    ].forEach(([label, feature, minutes]) => {
      const btn = document.createElement('button');
      btn.type = 'button'; btn.className = 'co-secondary'; btn.textContent = label;
      btn.addEventListener('click', async () => {
        try { await setOverride(child, feature, minutes); }
        catch (err) { view.renderError(err); }
      });
...
CompaniesView.prototype.buildMemberRow = function (c, m, roleNames, isAdmin, selectedIds, rebuildBulkBar) {
  const isProtected = m.role_id === 0 || m.role_id === 1;
...
CompaniesView.prototype.openMemberDetailsDialog = function (c, member) {
  ...
  const isProtected = member.role_id === 0 || member.role_id === 1;
  const roleNames = this.roleNameLookup();
...
CompaniesView.prototype.injectStyles = function () {
  ...
  const css = `
    ...
    .co-child-section { border: 1px solid var(--co-border); background: var(--panel); border-radius: 8px; padding: 12px; display: flex; flex-direction: column; gap: 12px; }
    .co-child-title-row { display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }
    .co-child-rule { border: 1px solid var(--co-divider); border-radius: 8px; padding: 10px; background: var(--panel-2); display: flex; flex-direction: column; gap: 8px; }
    .co-child-rule-head { display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap; }
    .co-child-state { color: var(--text-faint); font-size: 11.5px; }
    .co-child-inline { font-size: 12px; }
    .co-child-rule-grid { display: grid; grid-template-columns: repeat(3, minmax(120px, 1fr)); gap: 10px; }
    .co-child-days { display: flex; flex-wrap: wrap; gap: 6px; }
    .co-child-days label { display: inline-flex; align-items: center; gap: 4px; font-size: 11.5px; color: var(--text-dim); padding: 3px 8px; border: 1px solid var(--co-border); border-radius: 999px; background: var(--panel); }
    .co-child-days input { accent-color: var(--accent); }
    .co-child-actions { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
    @media (max-width: 760px) { .co-child-rule-grid { grid-template-columns: 1fr; } }
...
function formatTokenCountC(value) {
  const n = Number(value || 0);
  if (!isFinite(n)) return '0';
  if (Math.abs(n) >= 1_000_000_000) return (n / 1_000_000_000).toFixed(2).replace(/\.?0+$/, '') + 'B';
  if (Math.abs(n) >= 1_000_000) return (n / 1_000_000).toFixed(2).replace(/\.?0+$/, '') + 'M';
  return Math.round(n).toLocaleString();
}
function formatCompanyTokenBalanceC(c) {
  if (!c || c.token_balance == null) return '';
  const remaining = Number(c.token_balance || 0);
  const parts = [formatTokenCountC(remaining) + ' remaining'];
  if (c.token_limit != null) {
    const total = Number(c.token_limit || 0) + Number(c.topup_token_balance || 0);
    if (total > 0) parts.push('of ' + formatTokenCountC(total));
  }
  return parts.join(' ');
}

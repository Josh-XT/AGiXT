...
Enter a website URL then click `Train from Website` to train the agent on the website. The agent will scrape the website's information into its long term memory. This will allow the agent to answer questions about the website.
...
GitHub repository training allows you to enter the repository name, for example `Josh-XT/AGiXT`, then click `Train from GitHub Repository` to train the agent on the repository. There are options `Use a branch other than main` and to enter credentials if it is a private repository. You can also choose to use the agent's settings for the GitHub credentials if you have those stored in your agent settings.
...
- Uploads your new model to HuggingFace with your choice of `private_repo` on a `bool` once complete if your agent has a `HUGGINGFACE_API_KEY` in its config.

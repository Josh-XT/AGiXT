...
class CompanyGitHubBot:
    ...
    def _init_github_client(self):
        ...
        try:
            if self.github_app_id and self.github_app_private_key:
                ...
                self._is_ready = True
                logger.info(
                    f"GitHub bot for {self.company_name} initialized with App authentication"
                )
                ...
                self._is_ready = True
                logger.info(
                    f"GitHub bot for {self.company_name} initialized with token authentication"
                )
            else:
                logger.warning(
                    f"No GitHub credentials configured for {self.company_name}"
                )
        except Exception as e:
            logger.error(
                f"Failed to initialize GitHub client for {self.company_name}: {e}"
            )
            self._is_ready = False
    ...
    def verify_webhook_signature(self, payload: bytes, signature: str) -> bool:
        ...
        expected_signature = (
            "sha256="
            + hmac.new(
                self.github_webhook_secret.encode(), payload, hashlib.sha256
            ).hexdigest()
        )
    ...
    def _is_repo_allowed(self, repo_full_name: str) -> bool:
        ...
        if self.deployment_scope == DeploymentScope.SINGLE_REPO:
            ...
            return (
                repo_lower.startswith(f"{self.target_org.lower()}/")
                if self.target_org
                else False
            )
        elif self.deployment_scope == DeploymentScope.USER:
            return (
                repo_lower.startswith(f"{self.target_user.lower()}/")
                if self.target_user
                else False
            )
    ...
    async def _handle_issue_event(self, payload: dict) -> dict:
        ...
        should_auto_fix = self.auto_fix_issues and (
            action == "opened" or any(label in fix_labels for label in labels)
        )
...
""",
    ...
    async def _apply_code_changes(
        self, repo: Repository, branch_name: str, fix_response: str, issue_number: int
    ) -> str:
        ...
        for i, (file_path, code) in enumerate(
            zip(file_mentions[: len(code_blocks)], code_blocks)
        ):
    ...
    async def _generate_tests_for_pr(
        self, repo: Repository, pr: PullRequest, branch_name: str
    ):
        ...
        try:
            ...
            files = pr.get_files()
            changed_files = [
                f.filename for f in files if not f.filename.startswith("test")
            ]
...
Be constructive and specific in your feedback."""
            ...
            if "approve" in response_lower and "request changes" not in response_lower:
                ...
                "request changes" in response_lower or "needs changes" in response_lower
            ):
    ...
    async def _handle_issue_comment(self, payload: dict) -> dict:
        ...
        if (
            "@ai" in comment_body
            or "@bot" in comment_body
            or "ai assistant" in comment_body
        ):
    ...
    async def _handle_bot_command(
        self, repo_full_name: str, number: int, command: str, is_pr: bool = False
    ) -> dict:
        ...
        try:
            ...
            if "fix" in command_lower or "solve" in command_lower:
                ...
                if not is_pr:
                    return await self._attempt_issue_fix(
                        repo_full_name,
                        {
                            "number": number,
                            "title": item.title,
    ...
    async def _handle_pr_review_comment(self, payload: dict) -> dict:
        ...
        if "@ai" in comment_body or "@bot" in comment_body:
            ...
            repo_full_name = payload.get("repository", {}).get("full_name", "")
            return await self._handle_bot_command(
                repo_full_name, pr.get("number"), comment.get("body", ""), is_pr=True
            )
    ...
    async def _handle_pr_review(self, payload: dict) -> dict:
        ...
        if review.get("state") == "CHANGES_REQUESTED":
            ...
            return await self._address_review_feedback(
                repo_full_name, pr.get("number"), review.get("body", ""), context
            )

        return {"status": "acknowledged"}

    async def _address_review_feedback(
        self, repo_full_name: str, pr_number: int, feedback: str, context: PRContext
    ) -> dict:
...
Please provide the specific code changes needed to address this feedback."""
            ...
            changes = await self._apply_code_changes(
                ...
                context.branch_name,
                fix_response,
                context.issue_number or pr_number,
            )
    ...
    async def start(self):
        ...
        if self._is_ready:
            ...
            logger.warning(
                f"GitHub bot for {self.company_name} could not start - not ready"
            )
...
class GitHubBotManager:
        ...
        try:
            self._server_bot = CompanyGitHubBot(
                ...
                bot_permission_mode=config.get(
                    "github_bot_permission_mode", "allowlist"
                ),
                bot_owner_id=config.get("github_bot_owner_id"),
                bot_allowlist=config.get("github_bot_allowlist"),
                deployment_scope=config.get(
                    "github_bot_deployment_scope", "single_repo"
                ),
    ...
    async def _start_company_bot(
        self, company_id: str, company_name: str, config: dict
    ):

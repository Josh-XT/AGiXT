    ...
    def _authenticate(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(f"Failed to authenticate with iRobot API: {response.status_code}")
    ...
    async def schedule_cleaning(
        ...
        try:
            ...
            if self.failures < 3:
                self.failures += 1
                await asyncio.sleep(5)
                return await self.schedule_cleaning(day_of_week, hour, minute, cleaning_mode)
...
- Bin Full: {'Yes' if bin_full else 'No'}"""
    ...
    async def get_cleaning_history(self, days: int = 7) -> str:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/robots/{self.ROOMBA_ROBOT_ID}/history",
                params=params
            )
    ...
    async def get_battery_status(self) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                charge_time_remaining = battery_data.get("charge_time_remaining", "Unknown")
...
- Cleaning Power: {cleaning_power.title()}"""
            ...
            if self.failures < 3:
                self.failures += 1
                await asyncio.sleep(5)
                return await self.set_cleaning_preferences(cleaning_passes, edge_sweep, cleaning_power)

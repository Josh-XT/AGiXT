...
class Emailnator:
    def __init__(self, headers, cookies, domain=False, plus=True, dot=True, google_mail=False):
        ...
        for ads in self.s.post('https://www.emailnator.com/message-list', json={'email': self.email}).json()[
            'messageData']:
    ...
    def reload(self, wait=False, retry_timeout=5):
        ...
        while True:
            for msg in self.s.post('https://www.emailnator.com/message-list', json={'email': self.email}).json()[
                'messageData']:
    ...
    def open(self, msg_id):
        return self.s.post('https://www.emailnator.com/message-list',
                           json={'email': self.email, 'messageID': msg_id}).text
...
class Client:
    def __init__(self, headers, cookies):
        ...
        self.sid = json.loads(
            self.session.get(f'https://www.perplexity.ai/socket.io/?EIO=4&transport=polling&t={self.t}').text[1:])[
            'sid']
        ...
        assert self.session.post(
            ...
            data='40{"jwt":"anonymous-ask-user"}').text == 'OK', "Error authenticating"
    ...
    def search(self, query, mode='concise', focus='internet', file=None):
        ...
        assert focus in ['internet', 'scholar', 'writing', 'wolfram', 'youtube', 'reddit',
                         'wikipedia'], 'Search focus modes --> ["internet", "scholar", "writing", "wolfram", "youtube", "reddit", "wikipedia"]'
        ...
        if file:
            ...
            if not (upload_resp := requests.post(self._last_file_upload_info['url'], data=monitor,
                                                 headers={'Content-Type': monitor.content_type})).ok:
                raise Exception('File upload error', upload_resp)

            uploaded_file = self._last_file_upload_info['url'] + self._last_file_upload_info['fields']['key'].replace(
                '${filename}', 'myfile')
...
class PerplexityProvider:
    def __init__(
        ...
        self.PERPLEXITY_COOKIE_PATH = PERPLEXITY_COOKIE_PATH if PERPLEXITY_COOKIE_PATH else DEFAULT_PERPLEXITY_COOKIE_PATH
        self.EMAILNATOR_COOKIE_PATH = EMAILNATOR_COOKIE_PATH if EMAILNATOR_COOKIE_PATH else DEFAULT_EMAILNATOR_COOKIE_PATH
        self.FOCUS = FOCUS
        self.exec_nb = 5    # set to 5 as default to automatically renewed copilot

    def load_account(self,):
        try:
            with open(self.PERPLEXITY_COOKIE_PATH, "r",) as f:
    ...
    def reload_account(self):
        if self.exec_nb > 5:
            if path.isfile(self.EMAILNATOR_COOKIE_PATH):
                with open(self.EMAILNATOR_COOKIE_PATH, "r", ) as f:
                    curl = json.loads(f.read())
                cookies = curl['cookies']
                headers = curl['headers']
    ...
    async def instruct(self, prompt, tokens: int = 0):
        ...
        if self.perplexity_cli is None:
            self.load_account()
        self.reload_account()
        response = self.perplexity_cli.search(prompt, mode='concise', focus=self.FOCUS, )
...
if __name__ == '__main__':
    ...
    async def run_test():
        global response
        response = await perplexity.instruct("Tell me what's going on in the world today?")
        print(response)

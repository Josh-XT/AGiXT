...
class CompanyWhatsAppBot:
    ...
    async def _handle_admin_command(
        ...
        if cmd == "!help":
            ...
            current = self._get_selected_agent(phone_number) or await self._get_default_agent()
            ...
            current_agent = self._get_selected_agent(phone_number) or await self._get_default_agent()
    ...
    async def process_button_reply(self, sender_phone: str, button_id: str, button_title: str):
        """
...
class WhatsAppBotManager:
    ...
    def __init__(self):
        ...
        self.app_secret = getenv("FACEBOOK_APP_SECRET")  # Used for signature verification
            ...
            try:
                with get_session() as session:
                    settings = (
                        session.query(CompanyExtensionSetting)
                        .filter(
                            CompanyExtensionSetting.setting_name == "whatsapp_access_token",

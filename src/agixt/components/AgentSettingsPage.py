...
class AgentSettingsPage:
        ...
        if not agent_name:
            ...
            if add_agent_button:
                if new_agent_name:
                    ...
                    try:
                        Agent(new_agent_name).add_agent(
                            new_agent_name, provider_settings
                        )
            ...
            try:
                ...
                if provider_name:
                    provider_settings = self.render_provider_settings(
                        agent_settings, provider_name, st
                    )
                    agent_settings.update(provider_settings)
                st.subheader("Custom Settings")
                custom_settings = agent_settings.get("custom_settings", [])

                custom_settings_list = st.session_state.get(
                    "custom_settings_list", None
                )
    ...
    def render_provider_settings(self, agent_settings, provider_name: str, st):
        try:

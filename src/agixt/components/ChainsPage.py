...
class ChainsPage:
    def __init__(self, st):
        ...
        chain_action = st.selectbox(
            "Action", ["Create Chain", "Delete Chain", "Run Chain"]
        )
        ...
        if selected_chain_name:
            ...
            def modify_step(step):
                ...
                modify_agent_name = st.selectbox(
                    ...
                    index=(
                        [agent["name"] for agent in CFG.get_agents()].index(agent_name)
                        + 1
                    )
                ...
                if modify_prompt_type == "Command":
                    ...
                    if command_name:
                        command_args = Commands(agent_name).get_command_args(
                            command_name
                        )
